...
//				printRouteCodes(dimension, routeNumber);
			...
			if(flag==0 && (getRouteCapacity(demand, routeOfi) + getRouteCapacity(demand, routeOfj)) <= capacity)
			{
				vector<int> icopy=routeOfi;
				vector<int> jcopy=routeOfj;
                vector<int> rnumbercpy=routeNumber;
				float cos=calculateRouteCost(routeOfi,distanceTable)+calculateRouteCost(routeOfj,distanceTable);
				...
				cout<<"Route j";
				printRoute(routeOfj);
				auto it = std::find(routeOfi.begin(), routeOfi.end(), node.i);
				int idx = distance(routeOfi.begin(), it);
				int temp=routeOfi.size()-2;
				int y=routeOfi[idx];
				routeOfi[idx]=routeOfi[temp];
				routeOfi[temp]=y;

				cout<<"\n modified i is:-";
				printRoute(routeOfi);
				auto it1 = std::find(routeOfj.begin(), routeOfj.end(), node.j);
				int idx1 = distance(routeOfj.begin(), it1);
				int temp1=1;
				int y1=routeOfj[idx1];
				routeOfj[idx1]=routeOfj[temp1];
				routeOfj[temp1]=y1;
			...
			cout<<"After modifyin route is ";
			printRoute(routeOfi);
			if(calculateRouteCost(routeOfi,distanceTable)<cos)
			{
