...
TSPSolver::TSPSolver(vector<vector<int>> n,vector<int> d)
{
    nodes = n;
    depot=d;
}

double getDistance(vector<int> x,vector<int> y)
{
...
vector<vector<int>> TSPSolver::solveTSP()
{
    ...
    for (i = 0; i < numCities; i++)
    {
        ...
        {
            vector<int> x = nodes[i];
            vector<int> y = nodes[j];
